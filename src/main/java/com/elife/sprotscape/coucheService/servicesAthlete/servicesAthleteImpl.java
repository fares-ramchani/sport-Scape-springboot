...
public class servicesAthleteImpl implements servicesAthlete {
  ...
  public void ajouter_un_Athlete(AthleteRequestDTO AthleteRequestDTO) {
    ...
    jdbcUserDetailsManager.createUser(User.withUsername(AthleteRequestDTO.getNom())
        .password(passwordEncoder.encode(AthleteRequestDTO.getMotDePasse())).authorities("Athlete").build());
  ...
  public void modifierAthlete(AthleteRequestDTO AthleteRequestDTO) {
    ...
    if (AthleteRequestDTO.getMotDePasse() != "") {
      jdbcUserDetailsManager.updateUser(User.withUsername(AthleteRequestDTO.getNom())
          .password(passwordEncoder.encode(AthleteRequestDTO.getMotDePasse())).authorities("CLIENT").build());
    }
    AthleteRepository.save(Athlete);
  }
