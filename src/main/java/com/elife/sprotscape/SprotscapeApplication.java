    ...
    return args -> {
      if (!JdbcUserDetailsManager.userExists("fares"))
        JdbcUserDetailsManager.createUser(
            User.withUsername("fares").password(PasswordEncoder.encode("12345")).authorities("ADMIN").build());
      if (!JdbcUserDetailsManager.userExists("achraf"))
        JdbcUserDetailsManager.createUser(
            User.withUsername("achraf").password(PasswordEncoder.encode("12345")).authorities("ADMIN").build());
      if (!JdbcUserDetailsManager.userExists("aya"))
        JdbcUserDetailsManager.createUser(
            User.withUsername("aya").password(PasswordEncoder.encode("12345")).authorities("ADMIN").build());
